...
interface AccountsDataTableProps {
  ...
  purchaseProgress?: Record<
    string,
    { purchase_event_id: number; is_completed: boolean }
  >;
  isEditMode?: boolean;
  tempProgress?: {
...
export function AccountsDataTable({
  ...
  const purchasesProgressMap = tempProgress?.purchases as
    | { [key: string]: boolean }
    | undefined;
  ...
  const getDisplayDaysOffsetText = (
    col: TimelineColumnData,
  ): string | number => {
    if (col.kind === "split") {
      if (col.event?.kind === "purchase") {
        const offsetStr =
          col.event.daysOffset != null ? String(col.event.daysOffset) : "";
        if (col.event.isRestricted && col.event.maxDaysOffset)
          return `${offsetStr} (${col.event.maxDaysOffset})`;
        return offsetStr;
      }
  ...
  const renderCellContent = (
    col: TimelineColumnData,
    field: "token" | "name" | "daysOffset" | "timeSpent" | "accountDate",
  ) => {
    switch (field) {
      case "token":
  ...
  const getColumnSpecificStyle = (
    ...
    if (base.kind === "level")
      style = getColorStyle("level", base.isBonus, undefined, theme);
  ...
  const getSplitCellStyle = (
    sessionCompleted: boolean,
    eventCompleted?: boolean,
  ): React.CSSProperties => {
    const leftBg = sessionCompleted
      ? colors.completeScheduledStyle
      : colors.incompleteScheduledStyle;
    const rightBg =
      (eventCompleted ?? sessionCompleted)
        ? colors.completeScheduledStyle
        : colors.incompleteScheduledStyle;
    return {
      backgroundImage:
  ...
  const splitStateCache = useMemo(() => {
    const cache = new Map<
      string,
      { sessionCompleted: boolean; eventCompleted: boolean }
    >();
    ...
    accounts.forEach((account) => {
      columns.forEach((column) => {
        ...
        const eventKey = column.event
          ? `${account.id}_${column.event.id}`
          : null;
        const eventCompleted =
          column.event && eventKey
  ...
  const getDateCellStyle = (
    ...
    if (isEditMode && tempProgress) {
      ...
      if (col.kind === "level")
        return tempProgress.levels[cellKey]
          ? completeScheduledStyle
          : incompleteScheduledStyle;
      return (tempProgress.purchases as { [key: string]: boolean })[cellKey]
        ? completeScheduledStyle
        : incompleteScheduledStyle;
    }
    ...
    if (col.kind === "level")
      return levelsProgress[progressKey]?.is_completed
        ? completeScheduledStyle
        : incompleteScheduledStyle;
    return purchaseProgress[progressKey]?.is_completed
      ? completeScheduledStyle
      : incompleteScheduledStyle;
  };
  ...
  const renderCell = (
    ...
    const emitProgressChange = (
      ...
      const shiftRange = !!(event?.nativeEvent as MouseEvent | undefined)
        ?.shiftKey;
      onProgressChange?.(type, id, checked, {
        rangeFromStart: rangeFillMode || shiftRange,
        rangeId: rangeId ?? id,
      });
    };
    if (isEditMode && tempProgress && onProgressChange) {
      if (col.kind === "split") {
        ...
        const eventPurchaseId =
          col.event?.kind === "purchase" ? (col.event.id as number) : null;
        const purchaseDateOverride =
          eventPurchaseId != null && tempPurchaseDates
      ...
      if (col.kind === "level") {
        isCompleted =
          tempProgress.levels[cellKey] ??
          levelsProgress[cellKey]?.is_completed ??
          false;
      } else {
        const purchasesMap = tempProgress.purchases as unknown as {
          [key: string]: boolean;
        };
        isCompleted =
          purchasesMap[cellKey] ??
          purchaseProgress[cellKey]?.is_completed ??
          false;
      }

      const purchaseDateOverride =
        col.kind === "purchase" && tempPurchaseDates
      ...
      return (
        <div className="flex flex-col items-center gap-1">
              ...
              <Popover>
                ...
                <PopoverContent className="w-auto p-0" align="center">
                  <SimpleCalendar
                    ...
                    onDateSelect={(date) =>
                      onPurchaseDateChange?.(
                        acc.id * 100000 + (col.id as number),
                        date,
                      )
                    }
                    onClose={() => {}}
          ...
          {col.kind === "level" && (
            <span
              className={`text-xs ${isCompleted ? "line-through decoration-gray-500" : ""}`}
            >
              {renderTimelineCell(matrix[accIdx][colIdx])}
            </span>
          )}
  ...
  return (
    <Table>
      <TableHeader>
        <TableRow>
          <TableHead colSpan={3} style={headerStyle}>
            {t("levels.eventToken")}
          </TableHead>
          {columns.map((col) => {
            const columnStyle = getColumnSpecificStyle(col);
