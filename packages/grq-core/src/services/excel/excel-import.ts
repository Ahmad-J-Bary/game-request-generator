...
export async function importFromExcel(): Promise<{ success: boolean; message: string; imported: ImportData }> {
  try {
    ...
    if (!selected || Array.isArray(selected)) {
      return { success: false, message: 'No file selected', imported: { levels: [], purchaseEvents: [], accounts: [], progress: [], accountSessionDates: new Map() } };
    }
    ...
    return {
      ...
      imported: { levels: [], purchaseEvents: [], accounts: [], progress: [], accountSessionDates: new Map() }
    };
  }
}
